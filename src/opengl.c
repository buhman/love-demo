...
{
  int vertex_source_size;
  const char * vertex_source = read_file("shader/test.vert", &vertex_source_size);
  assert(vertex_source != NULL);
  int fragment_source_size;
  const char * fragment_source = read_file("shader/test.frag", &fragment_source_size);
  assert(fragment_source != NULL);

  test_program = compile(vertex_source, vertex_source_size,
